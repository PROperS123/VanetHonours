...
void DemoBaseApplLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    ...
    else if (EncryptedSafetyMessage* eBSM = dynamic_cast<EncryptedSafetyMessage*>(wsm)) {
        EV_ERROR << "POPULATE WSM" << endl;
        //
        // Here we need to create the encrypted message
        //
        // eBSM->eMSG(crypto_thingy(contents));
...
void DemoBaseApplLayer::handleSelfMsg(cMessage* msg)
{
    ...
    case SEND_BEACON_EVT: {
        ...
        EncryptedSafetyMessage* eBSM = new EncryptedSafetyMessage();
        // populateWSM(bsm);
        populateWSM(eBSM);
        sendDown(eBSM);
    ...
    case 3: {
        // need to add ENCRYPTED to the definitions as '3'
        //
        // Original code
        // DemoSafetyMessage* bsm = new DemoSafetyMessage();
        EncryptedSafetyMessage* eBSM = new EncryptedSafetyMessage();
        EV_ERROR << "WE GOT AN ENCRYPTED PACKET BOSS" << endl;
        //
        //
        // We are about to create a BSM packet
        //
        //
        populateWSM(eBSM);
        //
        // Here we need to create the encrypted packet
        //
        // WHICH WE DON'T KNOW HOW TO YET
        //

        // EncryptedBSMMessage* eBSM = encryptWSM(bsm);

        //
        // We are sending the BSM packet to be scheduled for transmissions
        //
        sendDown(eBSM);

        //
        // alternate sendDown required
        //
        // It will take our eBSM and put it into the queue instead of a cMessage
        //


        //
        // Possibly a new scheduleAt
        //
        // Or we could use an overdefine
        //
        scheduleAt(simTime() + beaconInterval, sendBeaconEvt);
        break;
...
void DemoBaseApplLayer::finish()
{
    ...
    recordScalar("generatedeBSMs", generatedeBSMs);
    recordScalar("receivedeBSMs", receivedeBSMs);
}
...
void DemoBaseApplLayer::checkAndTrackPacket(cMessage* msg)
{
    if (dynamic_cast<DemoSafetyMessage*>(msg)) {
        EV_ERROR << "sending down a BSM" << endl;
        generatedBSMs++;
    }
    else if (dynamic_cast<DemoServiceAdvertisment*>(msg)) {
        EV_ERROR << "sending down a WSA" << endl;
        generatedWSAs++;
    }
    else if (dynamic_cast<EncryptedSafetyMessage*>(msg)) {
        EV_ERROR << "sending down an eBSM" << endl;
        generatedeBSMs++;
    }
    else if (dynamic_cast<BaseFrame1609_4*>(msg)) {
        EV_ERROR << "sending down a WSM" << endl;
        generatedWSMs++;
    }
}
